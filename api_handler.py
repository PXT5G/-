...
class GenericWebhook:
    """Sends arbitrary JSON POST payloads to external relay/solver APIs."""
    ...
    def post_json(
        ...
        *,
        ...
        if not url or not url.strip():
            _log.warning("GenericWebhook: empty URL, skipping POST")
        ...
        try:
            ...
            _log.exception("GenericWebhook POST failed: %s", exc)
        ...
        if not ok:
            _log.warning("GenericWebhook: HTTP %s — %s", resp.status_code, text[:500])

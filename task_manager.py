...
from resource_webhook import BrowserTargets, ResourceErrorWebhookHandler
...
class AutomatedTestScenario:
    ...
    def _simulate_provider_checks(self, page: Page) -> None:
        ...
        if self._page_contains_any(page, ("sms", "phone verification", "verification code")):
            self._request_sms_metadata(page)
        else:
        ...
        if self._page_contains_any(page, ("captcha", "turnstile", "hcaptcha")):
            self._request_captcha_metadata(page)
        else:
            self._log("CAPTCHA signal not detected; skipping CAPTCHA metadata check.")

    def _trigger_resource_webhook(self, page: Page, message: str, error_type: str) -> None:
        """Notify universal webhook integration before surfacing a resource error."""
        try:
            handler = ResourceErrorWebhookHandler(
                browser=BrowserTargets(playwright_page=page),
            )
            handler.handle_resource_error(
                message,
                error_type=error_type,
                task_id=self.thread_label,
            )
        except Exception as exc:
            self._log(f"Webhook handler failed: {exc}")

    def _request_sms_metadata(self, page: Page) -> None:
        """Simulate provider readiness by checking 5sim account status only."""
        if not self.config.five_sim_api_key:
            msg = "Resource Error: 5sim key missing while phone verification is required."
            self._trigger_resource_webhook(page, msg, "sms")
        ...
        try:
            ...
            msg = "Resource Error: Phone verification requires manual sandbox fixture."
            self._trigger_resource_webhook(page, msg, "sms")
            raise TaskManagerError(msg) from exc

    def _request_captcha_metadata(self, page: Page) -> None:
        """Simulate solver readiness by checking CapSolver account status only."""
        if not self.config.capsolver_api_key:
            msg = "Resource Error: CapSolver key missing while CAPTCHA is required."
            self._trigger_resource_webhook(page, msg, "captcha")
            raise TaskManagerError(msg)
        ...
        try:
            ...
            msg = "Resource Error: CAPTCHA requires manual sandbox fixture."
            self._trigger_resource_webhook(page, msg, "captcha")
            raise TaskManagerError(msg) from exc

      ...
      <div className="grid grid-cols-2 gap-4">
        {openWindows.map((window, i) => {
          ...
          return (
            ...
            >
              <GlassPanel
                className="aspect-[9/16] p-2 cursor-pointer relative"
                onClick={() => { focusWindow(window.id); onClose(); }}
              >
                {bgApp?.pinned && (
                  <span className="absolute top-2 right-2 text-[10px] bg-gulf-gold/80 text-black px-1.5 py-0.5 rounded">
                    Pinned
                  </span>
                )}
                {bgApp?.frozen && (
                  <span className="absolute top-2 left-2 text-[10px] bg-blue-500/80 text-white px-1.5 py-0.5 rounded">
                    Frozen
                  </span>
                )}

...
const EVENT_COLORS: Record<string, string> = {
  ...
  bank_payment: 'bg-green-600', invitation: 'bg-banana-gold',
};
    ...
    <div className="h-full flex flex-col bg-black">
      <div className="p-4 border-b border-white/10 flex justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => { tap(); setShowNew(true); }} className="text-banana-gold text-2xl">+</button>
        </div>
      </div>
      ...
      {showNew && (
        <div className="absolute inset-0 bg-black/80 flex items-end">
          <div className="w-full p-4 bg-gray-900 rounded-t-3xl">
            ...
            <button type="button" onClick={() => create.mutate()} disabled={!title} className="w-full py-3 bg-banana-gold text-black rounded-xl font-semibold disabled:opacity-50">Create Event</button>
          </div>
        </div>
      )}
    </div>

    ...
    <div className="h-full flex flex-col bg-black">
      <div className="flex-1 overflow-y-auto p-4">
          ...
          <>
            {(alarms ?? []).map((a) => (
              <div key={String(a.alarmId)} className="flex justify-between items-center p-4 mb-2 rounded-xl bg-white/5">
                ...
                <button type="button" onClick={() => toggleAlarm.mutate(String(a.alarmId))} className={`w-12 h-7 rounded-full ${a.enabled ? 'bg-banana-gold' : 'bg-white/20'}`}>
                  <div className={`w-5 h-5 rounded-full bg-white transition-transform ${a.enabled ? 'translate-x-6' : 'translate-x-1'}`} />
                </button>
              </div>
            ))}
            <button type="button" onClick={() => createAlarm.mutate()} className="w-full py-3 mt-4 rounded-xl border border-dashed border-white/20 text-banana-gold text-sm">+ Add Alarm</button>
          </>
        )}
        {tab === 'stopwatch' && (
          <div className="text-center py-12">
            ...
            <div className="flex gap-4 justify-center">
              <button type="button" onClick={() => setSwRunning(!swRunning)} className="px-8 py-3 rounded-full bg-banana-gold text-black font-semibold">{swRunning ? 'Stop' : 'Start'}</button>
        ...
        {tab === 'timer' && (
          <div className="text-center py-12">
            ...
            <div className="flex gap-4 justify-center">
              <button type="button" onClick={() => setTimerRunning(!timerRunning)} className="px-8 py-3 rounded-full bg-banana-gold text-black font-semibold">{timerRunning ? 'Pause' : 'Start'}</button>
          ...
          (worldClocks ?? []).map((wc) => {
            ...
            return (
              <div key={wc.city} className="flex justify-between p-4 mb-2 rounded-xl bg-white/5">
                ...
                <span className="text-banana-gold tabular-nums">{formatTime(city)}</span>
              </div>
            );
          })
        )}
      ...
      <nav className="flex border-t border-white/10">
        {(['alarm', 'stopwatch', 'timer', 'world'] as Tab[]).map((t) => (
          <button key={t} type="button" onClick={() => { tap(); setTab(t); }} className={`flex-1 py-3 text-xs capitalize ${tab === t ? 'text-banana-gold' : 'text-white/40'}`}>{t === 'world' ? 'World' : t}</button>
        ))}
      </nav>
    </div>
  );

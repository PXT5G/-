...
export interface IAircraftInventory extends Document {
  inventoryId: string;
  dealerId: string;
  companyId: string;
  aircraftId: string;
...
const aircraftInventorySchema = new Schema<IAircraftInventory>(
  {
    ...
    hangarId: { type: String, index: true },
    ...auditSchemaFields,
  },
  { timestamps: true }
);

export const AircraftInventory = mongoose.model<IAircraftInventory>('AircraftInventory', aircraftInventorySchema);

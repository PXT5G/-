...
export interface IMarineAnalytics extends Document {
  analyticsId: string;
  dealerId?: string;
  companyId?: string;
  ...
  bestSellers: { vesselId: string; brand: string; model: string; unitsSold: number; revenue: number }[];
...
const marineAnalyticsSchema = new Schema<IMarineAnalytics>(
  {
    ...
    bestSellers: { type: [{ vesselId: String, brand: String, model: String, unitsSold: Number, revenue: Number }], default: [] },
...
export const MarineAnalytics = mongoose.model<IMarineAnalytics>('MarineAnalytics', marineAnalyticsSchema);
